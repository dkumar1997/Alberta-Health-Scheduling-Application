...
public class SQLQUERIES {
	...
	//TODO: write function to check if referral code is in database
	public boolean checkReferralCode(int referralNo) {
		try { 
			Connection con = getConnection();
			String query = String.format("SELECT * FROM referral WHERE referralCode in (%d)", referralNo);
			System.out.println(query);
			PreparedStatement getid = con.prepareStatement(query);
			ResultSet result = getid.executeQuery();
	...
	/**
	 * Check if referral code is in database and matches current user info
	 * @param referralNo
	 * @return
	 */
	public boolean checkReferralCode(int referralNo, int patientID) {
		return false;
	}
